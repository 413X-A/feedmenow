...
const progression = [
    ...
    { fortschritt: 2, feld: "Gemüsegarten", beschreibung: "Frischer Salat ist beliebt.", name: "Salat", dauer: 2, xp: 1, kaufpreis: 2, verkaufspreis: 4, ansehen: 1, freigeschaltet: false, wasserverbrauch: 1, energieverbrauch: 0, gold: 0, wasser: 0, energie: 0, samen: 0, geerntet: 0, bonus: false, fertiggewachsen: null, exp_fortschritt: 10, anzahlfelder: null },
...
function login() {
    ...
    function defaultBenutzer(passwort) {
        const daten = {
            ...
            aktuelleFarm: null,
            ressourcen: { gold: 0, wasser: 35, energie: 0, magie: 0, anz_pflanzen: 0, duenger: 0 },
            produkte: {},
            gebaeudeFelder: {}, // <--- hier speichern wir die Gebäude-Felder
            pflanzenDaten: {},
        ...
        magieFelder.forEach(p => {
            daten.pflanzenDatenMagische[p.fortschritt] = {
                ...p,
                felder: erzeugeFelder(p.anzahlfelder, p.fortschritt)
            };
        });

        // Produkte initialisieren + Gebäude-Felder erzeugen
        Object.keys(gebäudeProdukte).forEach(gebaeude => {
            daten.produkte[gebaeude] = gebäudeProdukte[gebaeude].map(p => ({ ...p }));

            // Gebäude-Felder erzeugen, falls anzahlfelder gesetzt
...
function prüfeUndAktualisiereBenutzerDaten(alleBenutzer, benutzername) {
    ...
    function objekteVergleich(a, b) {
        // freigeschaltet wird ignoriert
        const { freigeschaltet: _, ...restA } = a;
        const { freigeschaltet: __, ...restB } = b;
        return JSON.stringify(normalize(restA)) === JSON.stringify(normalize(restB));
    }

    function initialisiereFelderPflanze(pflanze) {
        if (!pflanze.felder) pflanze.felder = {};
        const anz = Number(pflanze.anzahlfelder) || 0;
        for (let i = 1; i <= anz; i++) {
            if (!pflanze.felder[i]) {
                pflanze.felder[i] = {
                    status: "leer",
    ...
    function initialisiereFelderGebäude(gebaeude) {
        ...
        if (gebaeude.anzahlfelder !== null && !daten.gebaeudeFelder[gebaeude.fortschritt]) {
            const felder = {};
            for (let i = 1; i <= gebaeude.anzahlfelder; i++) {
                felder[i] = {
                    fortschritt: gebaeude.fortschritt,
                    name: gebaeude.fortschritt,
                    status: "leer",
                    produkt: null,
                    zeit_start: null,
                    zeit_fertig: null
    ...
    Object.keys(gebäudeProdukte).forEach(gebaeudeName => {
        ...
        // Gebäude-Felder erzeugen
        gebäudeProdukte[gebaeudeName].forEach(p => {
            initialisiereFelderGebäude(p);
        });
    });
    ...
    function aktualisiereKategorie(speicher, quelle, label) {
        ...
        for (const key in aktuelleMap) {
            ...
            if (!gespeicherte) {
                ...
                initialisiereFelderPflanze(speicher[key]);
                hinzugefügt.push(`Neue ${label}: ${aktuelle.name}`);
            } else {
                if (!objekteVergleich(gespeicherte, aktuelle)) {
                    keysZuPruefen.forEach(k => {
                        gespeicherte[k] = aktuelle[k];
                    });
                    initialisiereFelderPflanze(gespeicherte);
        ...
        Object.keys(quelle).forEach(gebaeude => {
            if (!speicher[gebaeude]) {
                ...
                speicher[gebaeude] = speicher[gebaeude].map(prod => {
                    ...
                    const prodFiltered = Object.fromEntries(
                        Object.entries(prod).filter(([k]) => !keysToIgnore.includes(k))
                    );
                    const quelleFiltered = Object.fromEntries(
                        Object.entries(quelleProd).filter(([k]) => !keysToIgnore.includes(k))
                    );

                    const hatAenderung = !deepEqual(prodFiltered, quelleFiltered);

                    if (hatAenderung) {
                        const hergestelltAlt = prod.hergestellt ?? 0;
                        const anzahlAlt = prod.anzahlprodukte ?? 0;

                        const neuesProdukt = { ...quelleProd };
                        neuesProdukt.hergestellt = hergestelltAlt;
                        neuesProdukt.anzahlprodukte = anzahlAlt;
